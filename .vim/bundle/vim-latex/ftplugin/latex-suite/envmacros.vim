...
let s:captionbeside = "\\begin{figure}[h]\<cr>\\begin{minipage}[c]{0.7\\textwidth}\<cr>\\centering\<cr>\\includegraphics[width=\\textwidth]{<+file+>}\<cr>\\end{minipage}\<cr>\\begin{minipage}[c]{0.29\\textwidth}\<cr>\\caption{<+caption+>}\<cr>\\label{<+fig:label+>}\<cr>\\end{minipage}\<cr>\\end{figure}<++>"
...
call s:Tex_SpecialMacros('ECB', '', 'captionbeside', s:captionbeside)
" }}}
...
" }}} 
" Tex_captionbeside: {{{
function! Tex_captionbeside(env)
    return IMAP_PutTextWithMovement(s:captionbeside)
endfunction
" }}}
" Tex_table: {{{
...
function! Tex_SetFastEnvironmentMaps()
	if g:Tex_PromptedEnvironments != ''
        call Tex_MakeMap("<C-f>", "<Plug>Tex_FastEnvironmentInsert", 'i', '<silent> <buffer>')
		call Tex_MakeMap("<C-f>", "<Plug>Tex_FastEnvironmentInsert", 'n', '<silent> <buffer>')
		call Tex_MakeMap("<C-f>", "<Plug>Tex_FastEnvironmentInsert", 'v', '<silent> <buffer>')
